  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    describe(scenarioList[i] + ': sqlite version test(s)', function() {
      ...
      describe(suiteName + 'basic sqlite version test(s)', function() {
        ...
        it(suiteName + 'Check sqlite version (pattern ONLY for WebKit Web SQL, androidDatabaseImplementation: 2, and WP8)', function(done) {
          ...
          db.transaction(function(tx) {
            ...
            tx.executeSql('SELECT SQLITE_VERSION() AS myResult', [], function(tx_ignored, rs) {
              ...
              if (!isWebSql && !isWP8 && !(!isWindows && isAndroid && isImpl2))
                expect(rs.rows.item(0).myResult).toBe('3.8.10.2');
      ...
      describe(suiteName + 'sqlite encoding test(s)', function() {
        ...
        it(suiteName + 'Check internal database encoding: UTF-16le for Windows, UTF-8 for others (plugin ONLY) [WP8 TBD]', function(done) {
          ...
          db.executeSql('PRAGMA encoding', [], function(rs) {
            ...
            expect(rs.rows.length).toBe(1);
            if (isWP8) // TBD
              expect(rs.rows.item(0).encoding).toBeDefined(); // TBD
            else if (isWindows)
